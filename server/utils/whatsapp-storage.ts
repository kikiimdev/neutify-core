...
export type DefineWhatsAppStorageCallback<Device> = {
  ...
  onWhatsAppConnected?: (
    deviceId: string,
    whatsAppProfile: Contact
  ) => Promise<void>;
};

export async function defineWhatsAppStorage<Device>(
  callback: DefineWhatsAppStorageCallback<Device>
) {
  ...
  const device = {
    get: async (deviceId: string) => {
      try {
        let existingDevice: Device | null = await _storage.getItemRaw<Device>(
          `${devicePath}:${deviceId}`
        );
        ...
        if (!existingDevice) {
          ...
          if (existingDevice) {
            await _storage.setItemRaw(
              `${devicePath}:${deviceId}`,
              existingDevice as any
            );
          }
        }
  ...
  const socket = {
    ...
    onConnectionUpdate: async (
      deviceId: string,
      update: Partial<ConnectionState>
    ) => callback.onConnectionUpdate(deviceId, update),
    onWhatsAppConnected: async (deviceId: string, whatsAppProfile: Contact) =>
      callback.onWhatsAppConnected?.(deviceId, whatsAppProfile),
  };
...
export type DefineWhatsAppStorage<Device> = ReturnType<
  typeof defineWhatsAppStorage<Device>
>;
